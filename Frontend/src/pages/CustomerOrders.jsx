...
import { FaStar, FaMotorcycle, FaUtensils, FaThumbsUp, FaThumbsDown } from 'react-icons/fa';
import { useModal } from '../context/ModalContext';
import RatingModal from '../components/RatingModal'; // Assuming RatingModal handles both restaurant and driver
...
const CustomerOrders = () => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            // For each delivered order, we need to get the restaurant and driver details for the modal
            const ordersWithDetails = await Promise.all(response.data.map(async order => {
                if (order.status === 'delivered' && !order.restaurantDetails) {
                    try {
                        ...
                        console.warn(`Could not fetch details for order ${order._id}:`, err);
                    }
                }
                return order;
            }));
    ...
    const handleRatingSubmit = async (data, orderId) => {
        const { restaurantRating, driverRating, restaurantLikeStatus, driverLikeStatus } = data;
        const sessionKey = Object.keys(sessionStorage).find(key => key.startsWith('token_'));
        const token = sessionKey ? sessionStorage.getItem(sessionKey) : null;
        if (!token) {
            showAlert('Authentication required to submit rating.');
            return;
        }

        try {
            await axios.post(`http://localhost:3000/api/orders/${orderId}/submit-rating`, {
                restaurantRating,
                driverRating,
            ...
            showAlert('Ratings submitted successfully!');
            fetchOrders(); // Refresh orders to show updated rating status
        } catch (err) {
            const msg = err.response?.data?.error || 'Failed to submit ratings.';
            showAlert(`Error: ${msg}`);
            console.error('Frontend (CustomerOrders) - Rating submission error:', err.response ? err.response.data : err);
        }
    };
                ...
                <div className="space-y-8">
                    {orders.map((order) => (
                        <div key={order._id} className="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
                            ...
                            {/* NEW: Rating section for delivered orders */}
                            {order.status === 'delivered' && (!order.restaurantRated || (order.deliveryPersonId && !order.driverRated)) && (
                            ...
                            {order.status === 'delivered' && order.restaurantRated && (
